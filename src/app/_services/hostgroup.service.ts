...
export class HostgroupService {
  ...
  public getHostgroups(token: string): Observable<Hostgroup[]> {
    return this.http.post<Hostgroup[]>('http://192.168.5.86/enigma-api/hostgroups/get', JSON.stringify({ token: token })).map(
  ...
  public getHostgroup(token: string, id: number): Observable<Hostgroup[]> {
    return this.http.post<Hostgroup[]>('http://192.168.5.86/enigma-api/hostgroups/get/' + id, JSON.stringify({ token: token, id: id })).map(
  		res => res.map(x => new Hostgroup(x.id, x.name, x.ip, x.port, x.status, x.state, x.description, x.default_group, x.groups, x.hosts_down, x.hosts_pending,
                                             x.hosts_unreachable, x.hosts_up, x.services_crit, x.services_ok, x.services_pending, x.services_unknown,
                                             x.services_warn)));
  }
  ...
  public setDefaultGroup(token: string, id: number, groupName: string): Observable<boolean> {
    return this.http.put('http://192.168.5.86/enigma-api/hostgroups/set/' + id, JSON.stringify({ value: groupName })).map((response: Response) => {
      return true;
    });
  }
  ...
  public getCustomers() : Observable<Customer[]> {
    return this.http.get<Customer[]>('http://192.168.5.86/enigma-api/customers/get').map(
    res => res.map(x => new Customer(x.id, x.name, x.logo)));
  }

  public getCustomer(id: number) : Observable<Server[]> {
    return this.http.get<Server[]>('http://192.168.5.86/enigma-api/customers/get/' + id).map(
  ...
  public createCustomer(name: string, logo: any): Observable<any> {
    return this.http.post<any>('http://192.168.5.86/enigma-api/customers/create', JSON.stringify({ name: name, logo: logo}))
  ...
  public createServer(description: string, address: string, port: number, customer_id: number): Observable<any> {
    return this.http.post<any>('http://192.168.5.86/enigma-api/hostgroups/create', JSON.stringify({ description: description, address: address, port: port, customer_id: customer_id}))
    .map((response: HttpResponse<any>) => {
      return response;
    });
  }
