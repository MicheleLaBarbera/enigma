...
export class AuthenticationService {
  ...
  login(username: string, password: string): Observable<boolean> {
    return this.http.post('http://localhost/192.168.5.86/users/auth', JSON.stringify({ username: username, password: password }))
  ...
  signup(firstname: string, lastname: string, username: string, password: string, customer: number, email: string): Observable<any> {
    return this.http.post<any>('http://localhost/192.168.5.86/users/create', JSON.stringify({ firstname: firstname, lastname: lastname, username: username, password: password, customer: customer, email: email}))
    .map((response: HttpResponse<any>) => {
      return response;
    });
  }
