...
export class HostgroupService {
  ...
  public setDefaultGroup(token: string, id: number, groupName: string): Observable<boolean> {
    return this.http.put('http://localhost/api/hostgroups/set/' + id, JSON.stringify({ value: groupName })).map((response: Response) => {
      return true;
    });
  }

  public getHostsByState(state: number) : Observable<Host[]> {
    return this.http.post<Host[]>('http://localhost/api/hosts/state', JSON.stringify({ state: state })).map(
    res => res.map(x => new Host(x.ip, x.name, '', x.services_crit, x.services_ok, x.services_unknown, x.services_warn, '', '', 0)));
  }
}
